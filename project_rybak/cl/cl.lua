    ...
    if IsEntityInWater(PlayerPedId()) or IsEntityInWater(boat) then
        if not IsPedInAnyVehicle(PlayerPedId(), false) then
            if not IsPedSwimming(PlayerPedId()) then
                ...
                draw = true
                rng22 = true
                ESX.ShowNotification('Łowienie...')
                TriggerEvent('s_fishing:rng')
                fishing()
            else
                ESX.ShowNotification('Musisz zdecydować czy chcesz pływać czy łowić ryby')
            end
        end
    else
        ESX.ShowNotification('Nie możesz tutaj łowić')
    end
end)
...
function drawtext()
    ...
    while draw do
        ...
        Draw3DText(pedcoords.x - 1, pedcoords.y, pedcoords.z + 1,'Naciśnij [~r~X~s~] by przestać łowić')
        if IsControlJustPressed(0, 73) then
            ESX.ShowNotification('Przestano łowić')
            DeleteEntity(rod)
            DeleteObject(rod)
            rng22 = false
            TriggerEvent('s_fishing:rng')
    ...
    if rng22 then
        TriggerEvent('s_fishing:minigame', function(success)
            if success then
                ...
                ClearPedTasks(PlayerPedId())
                TriggerEvent('s_fishing:rng')
                ESX.ShowNotification("Nic nie złapałeś")
            end
        end)
    end
end
